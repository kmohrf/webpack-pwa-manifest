...
const supportedMimeTypes = [_jimp.default.MIME_PNG, _jimp.default.MIME_JPEG, _jimp.default.MIME_BMP];
...
function processIcon(currentSize, icon, buffer, mimeType, publicPath, shouldFingerprint) {
  ...
  const fileName = shouldFingerprint ? `icon_${dimensions}.${(0, _fingerprint.default)(buffer)}.${_mime.default.getExtension(mimeType)}` : `icon_${dimensions}.${_mime.default.getExtension(mimeType)}`;
  const iconOutputDir = icon.destination ? (0, _uri.joinURI)(icon.destination, fileName) : fileName;
  const iconPublicUrl = (0, _uri.joinURI)(publicPath, iconOutputDir);
  return {
    manifestIcon: {
      src: iconPublicUrl,
      sizes: dimensions,
...
function process(sizes, icon, cachedIconsCopy, icons, assets, fingerprint, publicPath, callback) {
  const processNext = function processNext() {
    if (sizes.length > 0) {
  ...
  if (size > 0) {
    const mimeType = _mime.default.getType(icon.src);
    ...
    if (!supportedMimeTypes.includes(mimeType)) {
      ...
      try {
        ...
        throw new _IconError.default(`It was not possible to read '${icon.src}'.`);
      }
    ...
    _jimp.default.read(icon.src, (err, img) => {
      if (err) throw new _IconError.default(`It was not possible to read '${icon.src}'.`);
      img.resize(size, size).getBuffer(mimeType, (err, buffer) => {
        if (err) throw new _IconError.default(`It was not possible to retrieve buffer of '${icon.src}'.`);
        const processedIcon = processIcon(size, icon, buffer, mimeType, publicPath, fingerprint);
        icons.push(processedIcon.manifestIcon);
        assets.push(processedIcon.webpackAsset);
        return processNext();
