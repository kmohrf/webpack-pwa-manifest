...
function createFilename(filenameTemplate, json, shouldFingerprint) {
  const formatters = [{
    ...
    value: (match, limit = ':32') => {
      ...
      const hash = (0, _fingerprint.default)(json);
...
function manifest(options, publicPath, icons, callback) {
  const content = (0, _except.default)(Object.assign({
    icons
  ...
  const file = _path.default.parse(options.filename);

  const filename = createFilename(file.base, json, options.fingerprints);
  const output = options.includeDirectory ? _path.default.join(file.dir, filename) : filename;
  callback(null, {
    output,
...
function buildResources(_this, publicPath, callback) {
  if (_this.assets && _this.options.inject) {
    ...
    (0, _icons.parseIcons)(_this.options.fingerprints, publicPath, (0, _icons.retrieveIcons)(_this.options), (err, result) => {
      if (err) return;
